...
$adaux = "../../php/"; include("../../php/iniciar.php"); include('../../php/admin.php'); $ad = new admin(); $seccion = 28;

if($_POST['admin_lis_usu_php_op'] != ""){
        switch($_POST['admin_lis_usu_php_op']){
            case 1:{ $sql = $ini->consulta("select * from url_amg where seccion = '".$seccion."'");  while($reg = $ini->fetch_object($sql)){ if(isset($_POST['chusu'.$reg->id])){
                     $ssql = $ini->consulta("update url_amg set activo = '1' where id = '".$reg->id."'"); } } }; break;
            case 2:{ $sql = $ini->consulta("select * from url_amg where seccion = '".$seccion."'");  while($reg = $ini->fetch_object($sql)){ if(isset($_POST['chusu'.$reg->id])){
                     $ssql = $ini->consulta("update url_amg set activo = '0' where id = '".$reg->id."'"); } } }; break;
            case 3:{ $sql = $ini->consulta("select * from url_amg where seccion = '".$seccion."'");  while($reg = $ini->fetch_object($sql)){
                     if(isset($_POST['chusu'.$reg->id])){ 
                        $ssql = $ini->consulta("delete from url_amg where id = '".$reg->id."'"); 
                        $ssql = $ini->consulta("delete from negocio where id_negocio = '".$reg->id."'");
                        $ssql = $ini->consulta("select * from slide where tipo = '".$reg->id."'");
                        while($rreg = $ini->fetch_object($ssql)){
                            $sqql = $ini->consulta("delete from slide_lang where id_slide = '".$rreg->id."'");
                        }
                        $ssql = $ini->consulta("delete from slide where tipo = '".$reg->id."'");
                        $ssql = $ini->consulta("delete from union_ngc_crvz where id_negocio = '".$reg->id."'");
                        $ssql = $ini->consulta("delete from union_ngc_ctgr where id_negocio = '".$reg->id."'");
                        $ssql = $ini->consulta("delete from union_ngc_lcld where id_negocio = '".$reg->id."'");
                    } } }; break;
            ...
            case 6:{ 
                ...
                $ssql = $ini->consulta("delete from cerveza where id_cerveza = '".$_POST['admin_lis_usu_php_id']."'");
                $ssql = $ini->consulta("select * from slide where tipo = '".$_POST['admin_lis_usu_php_id']."'");
                while($rreg = $ini->fetch_object($ssql)){
                    $sqql = $ini->consulta("delete from slide_lang where id_slide = '".$rreg->id."'");
                }
                $ssql = $ini->consulta("delete from slide where tipo = '".$_POST['admin_lis_usu_php_id']."'");
                $ssql = $ini->consulta("delete from union_ngc_crvz where id_negocio = '".$_POST['admin_lis_usu_php_id']."'");
                $ssql = $ini->consulta("delete from union_ngc_ctgr where id_negocio = '".$_POST['admin_lis_usu_php_id']."'");
                $ssql = $ini->consulta("delete from union_ngc_lcld where id_negocio = '".$_POST['admin_lis_usu_php_id']."'");
...
if($_POST['encuentra'] != 1){
    ...
    if(isset($_POST['busqueda']) and $_POST['busqueda'] != "" or isset($_SESSION['busqueda'])){  
        ...
        $buscarsql = ", match(titulo,metas,contenido) against ('".$_POST['busqueda']."') as buscador"; $buscarsqll = " where match(titulo,metas,contenido) against ('".$_POST['busqueda']."') and seccion = '".$seccion."' and lang = '".$_POST['idioma']."'"; 
        ...
        if($_POST['tipoorden'] == ""){
            $sql = $ini->consulta("select * from url_amg inner join url_amg_lang on url_amg.id = url_amg_lang.id_url_amg where lang = '".$_POST['idioma']."' and seccion = '".$seccion."' order by fecha desc limit ".$_POST['desde'].",".$_POST['hasta']);
        }else{
            $sql = $ini->consulta("select * from url_amg inner join url_amg_lang on url_amg.id = url_amg_lang.id_url_amg where lang = '".$_POST['idioma']."' and seccion = '".$seccion."' order by ".$arrord[$_POST['tipoorden']]." ".$oras." limit ".$_POST['desde'].", ".$_POST['hasta']);
        }
    }
}else{
    $sql = $ini->consulta("select * from url_amg inner join url_amg_lang on url_amg.id = url_amg_lang.id_url_amg where lang = '".$_POST['idioma']."' and plantilla = '".$_POST['idencuentra']."' and seccion = '".$seccion."' order by fecha desc");
}
...
<?php while($reg = $ini->fetch_object($sql)){ ?>
    ...
    <tr>
        ...
        <td><a href="admin.php?op=<?php echo $_POST['op']; ?>&se=5&id=<?php echo $reg->id; ?>"><?php echo $reg->titulo; ?></a></td>
