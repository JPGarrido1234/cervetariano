...
if($_POST['admin_lis_usu_php_op'] != ""){
        switch($_POST['admin_lis_usu_php_op']){
            case 1:{ $sql = $ini->consulta("select * from url_amg where seccion = '32'");  while($reg = $ini->fetch_object($sql)){ if(isset($_POST['chusu'.$reg->id])){
                     $ssql = $ini->consulta("update url_amg set activo = '1' where id = '".$reg->id."'"); } } }; break;
            case 2:{ $sql = $ini->consulta("select * from url_amg where seccion = '32'");  while($reg = $ini->fetch_object($sql)){ if(isset($_POST['chusu'.$reg->id])){
...
if($_POST['encuentra'] != 1){
    ...
    if(isset($_POST['busqueda']) and $_POST['busqueda'] != "" or isset($_SESSION['busqueda'])){  
        ...
        $buscarsql = ", match(titulo,metas,contenido) against ('".$_POST['busqueda']."') as buscador"; $buscarsqll = " where match(titulo,metas,contenido) against ('".$_POST['busqueda']."') and lang = '".$_POST['idioma']."' and seccion = '32'"; 
        ...
        if($_POST['tipoorden'] == ""){
            $sql = $ini->consulta("select * from url_amg inner join url_amg_lang on url_amg.id = url_amg_lang.id_url_amg where lang = '".$_POST['idioma']."' and seccion = '32' order by fecha desc limit ".$_POST['desde'].",".$_POST['hasta']);
        }else{
            $sql = $ini->consulta("select * from url_amg inner join url_amg_lang on url_amg.id = url_amg_lang.id_url_amg where lang = '".$_POST['idioma']."' and seccion = '32' order by ".$arrord[$_POST['tipoorden']]." ".$oras." limit ".$_POST['desde'].", ".$_POST['hasta']);
        }
    }
}else{
