...
$sql = $ini->consulta("select * from url_amg where seccion = '26'"); if($ini->num_rows($sql) > 0){ 
if(isset($_GET['npagt'])){ $paginar = $_GET['npagt']; }else{ $paginar = 10; } $desde = 0;
if($_GET['pagina'] != ""){ $desde = ($_GET['pagina']-1)*$paginar; } $hasta = $paginar; 
$sql = $ini->consulta("select * from url_amg where seccion = '26' order by fecha desc"); $contador = $ini->num_rows($sql);
?>
...
function marcartodos(){ 
    if(document.getElementById("seltodos").checked == true){ 
        <?php $sql = $ini->consulta("select * from url_amg where seccion = '26' order by fecha desc limit ".$desde.",".$hasta);  while($reg = $ini->fetch_object($sql)){ ?>
...
<?php $sql = $ini->consulta("select * from url_amg inner join url_amg_lang on url_amg.id = url_amg_lang.id_url_amg where lang = '".$idioma."' and url_amg.seccion = '26' order by fecha desc limit ".$desde.",".$hasta); while($reg=$ini->fetch_object($sql)){ ?>
    <tr>
        <td>
            <div class="icheckbox_flat-blue" aria-checked="false" aria-disabled="false" style="position:relative">
