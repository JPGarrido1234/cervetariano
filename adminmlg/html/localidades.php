...
<div class="panel panel-warning">
<div class="panel-heading"> <i class="fa fa-rss fa-fw"></i> Provincia padre</div>
    <div class="panel-body">
        <div class="form-group row">
            <div class="box">
                <select name="provincia_padre" id="idprovincia_padre" class="form-control" required>
                    <option value="0">Selecciona Provincia</option>
                    <?php $sqla = $ini->consulta("select id, provincia from provincias order by provincia asc"); while($rega = $ini->fetch_object($sqla)){ ?>
    ...
    <div class="panel-body">
        <div class="form-group row">
            <div class="input__row uploader">
              ...
              <input type="file" class="upload" id="file_1" name="archivo" value="" />
            </div>
        </div>
        <div class="form-group row">
            <input type="text" class="form-control" id="iduimg" name="uimg" value="" />
            <label for="iduimg">¿Añadir a traves de una url?</label>
        </div>
        <div class="form-group row">
...
<?php $sql = $ini->consulta("select * from url_amg"); if($ini->num_rows($sql) > 0){ ?>
...
$sql = $ini->consulta("select * from url_amg order by fecha desc"); $contador = $ini->num_rows($sql); ?>
...
function buscador(id,se,op){
    ...
    if(op == 1){
        ...
        var url = "../utiles/admin/php/muestra_filas_localidades.php"; 
...
<?php $sql = $ini->consulta("select * from url_amg inner join url_amg_lang on url_amg.id = url_amg_lang.id_url_amg where lang = '".$idioma."' and url_amg.seccion = '32' order by fecha desc limit ".$desde.",".$hasta); while($reg=$ini->fetch_object($sql)){ ?>
    <tr>
        <td>
            <div class="icheckbox_flat-blue" aria-checked="false" aria-disabled="false" style="position:relative">
...
while($regcr=$ini->fetch_object($sqlcr)){ $arbica[$regcr->id] = array($regcr->id,$regcr->carpeta,$regcr->nombre_amg); } 
$sqlb = $ini->consulta("select IdProvincia from municipios where id_url_amg = '".$_GET['id']."'"); if($ini->num_rows($sqlb) > 0){ $regb = $ini->fetch_object($sqlb); $provincia = $regb->IdProvincia; }else{ $provincia = 0; }
...
<div class="panel-heading"> <i class="fa fa-rss fa-fw"></i> Provincia padre</div>
    <div class="panel-body">
        <div class="form-group row">
            <div class="box">
                <select name="categoria" id="idcategoria" class="form-control" required>
                    <option value="0">Selecciona Provincia</option>
                    <?php $sqla = $ini->consulta("select id, provincia from provincias order by provincia asc"); while($rega = $ini->fetch_object($sqla)){ ?>
                        <option<?php if($rega->id == $provincia){ ?> selected="selected"<?php } ?> value="<?php echo $rega->id; ?>"><?php echo $rega->provincia; ?></option>
                    <?php } ?>
                </select>
            </div>
        </div>
