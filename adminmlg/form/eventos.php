...
switch($_POST['modo']){
	case 'crear' :{
		...
		$sqlurlamg = $ini->consulta("select id_url_amg from url_amg_lang where url_amg = '".$url_amg."' and seccion = '23'"); 
		$url_amgfn = $url_amg; $i=1; 
		while($ini->num_rows($sqlurlamg) > 0){
			$url_amgfn = $url_amg.$i; $i++; 
			$sqlurlamg = $ini->consulta("select id_url_amg from url_amg_lang where url_amg_lang = '".$url_amgfn."' and seccion = '23'"); 
		} 
		$url_amg = $url_amgfn;
		$imagen = "";
		$titalt = "";
		...
		$sql = $ini->consulta("insert into url_amg (seccion,subsecc,plantilla,fecha,thumbs,activo) values ('23','0','14','".$fecha."','".$imagen."','".$activo."')");
		...
		$ssql = $ini->consulta("insert into eventos (id_evento,fecha) value ('".$ultimoid."','".$_POST['fecha']."')");
		$i = 1;
		$se = 5; 
		$idg = "&id=".$ultimoid;
	};
	...
	case 'modificar' :{
		...
		$date = explode("/", $_POST['fecha']);
		$ssql = $ini->consulta("insert into eventos (id_evento,fecha) value ('".$_POST['id']."','".$_POST['fecha']."')");
		$ssql = $ini->consulta("update eventos set fecha = '".$_POST['fecha']."' where id_evento = '".$_POST['id']."'");
