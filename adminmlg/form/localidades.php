	...
	case 'crear' :{
		...
		if($_FILES['archivo']['tmp_name'] != ""){
			if($_POST['nueva'] != ""){  
				...
				$ssql = $ini->consulta("insert into carpetas (carpeta,nombre_amg) value ('/".$carpeta."/','".$ini->real_scape($_POST['nueva'])."')"); 
			}
			if($carpeta != ""){ 
				$dir = "/".$carpeta."/"; 
			}else{ 
		...
		$sql = $ini->consulta("insert into url_amg (seccion,subsecc,plantilla,fecha,thumbs,activo) values ('32','0','11','".$fecha."','".$imagen."','".$activo."')");
		...
		$muni_amg = $ad->cambiaralfn($_POST['name']);
		$ssql = $ini->consulta("insert into municipios (IdProvincia,Nombre,url_amg,id_url_amg) value ('".$_POST['provincia_padre']."','".$ini->real_scape($_POST['name'])."','".$muni_amg."','".$ultimoid."')");
		$se = 5; 
		$idg = "&id=".$ultimoid;
	};
	break;	
