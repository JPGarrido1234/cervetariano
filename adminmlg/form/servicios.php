...
switch($_POST['modo']){
	case 'crear' :{
		...
		$sql = $ini->consulta("insert into url_amg (seccion,subsecc,plantilla,fecha,thumbs,activo) values ('51','0','11','".$fecha."','".$imagen."','".$activo."')");
		...
		$ssql = $ini->consulta("insert into servicio (id_servicio,icono) value ('".$ultimoid."','".$_POST['icono']."')");
	...
	case 'modificar' :{
		...
		if($_FILES['imgt']['tmp_name'] != ""){
			...
			$sqql = $ini->consulta("insert into servicio (id_servicio,icono) value ('".$_POST['id']."','".$_POST['icono']."')");

			$sqql = $ini->consulta("update servicio set icono = '".$_POST['icono']."' where id_servicio = '".$_POST['id']."'");
